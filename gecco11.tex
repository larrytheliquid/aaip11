...
which has one additional output.

In the \texttt{not} rule the premise requires that the previous output
be more than just any arbitrary \texttt{n}. Instead, the previous
...
\texttt{not} allows us to infer the existance of the another term of
the same size (\texttt{1 + n}). This fits with our informal mental
...
\subsection{Extending To Richer Languages}
