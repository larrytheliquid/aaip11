...
hand, `and` requires the input stack to have at least two elements,
...
constant. Yet, it is not hard to imagine an off-by-1 error that drops
an individual somewhere, and maybe even another that adds a different
individual back somewhere else. On the more extreme end, a population
may be halved at some point and double at another, leaving the black
box obversation of the final population correct. Such examples leave
...
looking at runtime-specific implementation details. We can see in TODO
...
the polymorphic parameter `A` ensures that the type of lists/vectors
does not vary between the input and output, the natural number
parameter `n` ensures that the length of the population remains
constant.
