...
to trees. As such, we will concentrate on a developing a stack-based
...
GP:
\begin{itemize}
\item{\textbf{parameters:}} Starting with an non-dependently typed
...
\item{\textbf{representation:}} Next we will see how to use precise
  dependent types to encode arity and type information for candidate
  programs.
\item{\textbf{evaluation function:}} Taking advantage of the host
  ...
  an evaluation function for terms that is assured to terminate and
  not diverge.
\item{\textbf{genetic operators:}} Finally we will see what types and
  functions are necessary to ensure that only type-safe programs ever
  appear in the population after crossover.
\item{\textbf{initialization procedure:}} Due to space constraints, we
  will omit the initialization function (although it can be found in
