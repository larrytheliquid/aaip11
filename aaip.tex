...
\begin{verbatim}
  data Term (m : ℕ) : ℕ → Set where
    []   : Term inp
    true : {n : ℕ} → Term m      n  → Term m (1 + n)
    not  : {n : ℕ} → Term m (1 + n) → Term m (1 + n)
    and  : {n : ℕ} → Term m (2 + n) → Term m (1 + n)
\end{verbatim}
...
some value \texttt{m} and produces a stack of same size, acting as
...
value \texttt{m}, and produces another arbitrary value \texttt{n},
...
see that within any given subterm the \texttt{m} input remains the same.
